...
if [ -n "$ROBOT_NS" ]; then
	ros2 launch go2_sdk slam_launch.py robot_namespace:=$ROBOT_NAME existing_map:=$SLAM_MAP_NAME &
else
	ros2 launch go2_sdk slam_launch.py existing_map:=$SLAM_MAP_NAME &
fi
...
if [ -n "$ROBOT_NS" ]; then
    ros2 launch go2_sdk nav2_launch.py robot_namespace:=$ROBOT_NAME &
else
    ros2 launch go2_sdk nav2_launch.py &
fi
