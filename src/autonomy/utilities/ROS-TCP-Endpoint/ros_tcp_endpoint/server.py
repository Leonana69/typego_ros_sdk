...
class TcpServer(Node):
    ...
    def setup_executor(self):
        ...
        try:
            ...
            # Handle InvalidHandle and other executor errors gracefully
            if "InvalidHandle" in str(type(e).__name__) or "cannot use Destroyable" in str(e):
                self.logwarn("Executor encountered handle error (likely during shutdown): {}".format(e))
            else:
                raise
            ...
            with self._executor_lock:
                if self.executor is not None:
                    try:
                        ...
                        # Node may already be removed or destroyed
                        self.logwarn("Error removing node from executor: {}".format(e))
            try:
                old_node.unregister()
            except Exception as e:
    ...
    def destroy_nodes(self):
        ...
        """
        with self._executor_lock:
            # Remove all nodes from executor before destroying
            if self.executor is not None:
                for ros_node in list(self.publishers_table.values()):
                    try:
                        self.executor.remove_node(ros_node)
                    except Exception:
                        pass
        ...
        if self.tcp_server.executor is not None:
            with self.tcp_server._executor_lock:
                try:
                    ...
                    self.tcp_server.logwarn("Error adding subscriber node to executor: {}".format(e))
        ...
        if self.tcp_server.executor is not None:
            with self.tcp_server._executor_lock:
                try:
                    ...
                    self.tcp_server.logwarn("Error adding publisher node to executor: {}".format(e))
        ...
        if self.tcp_server.executor is not None:
            with self.tcp_server._executor_lock:
                try:
                    ...
                    self.tcp_server.logwarn("Error adding ROS service node to executor: {}".format(e))
        ...
        if self.tcp_server.executor is not None:
            with self.tcp_server._executor_lock:
                try:
                    ...
                    self.tcp_server.logwarn("Error adding Unity service node to executor: {}".format(e))
